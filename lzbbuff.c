...
static inline lzbbuff_hash fnv_1a_hash(size_t key_size, const uint8_t *key){
...
static inline lzbbuff_byte *align_ptr(size_t alignment, lzbbuff_byte *ptr){
...
void byte_as_hex(lzbbuff_byte byte){
    switch (byte){
        case 10:{
            printf("a");
            break;
...
void lzbbuff_print_as_hex(const LZBBuff *buff){
    ...
    lzbbuff_byte *bytes = buff->raw_buff;
    ...
    for (size_t i = 0; i < len; i++){
        lzbbuff_byte byte = bytes[i];
        lzbbuff_byte higher = (byte >> 4) & 0xf;
        lzbbuff_byte lower = byte & 0xf;
